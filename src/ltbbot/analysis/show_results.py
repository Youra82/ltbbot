...
# Suppress verbose TensorFlow/Keras logs (though TF is not used, keep for safety)
...
# --- ltbbot Imports ---
# Verwende den Envelope Backtester
from ltbbot.analysis.backtester import load_data, run_envelope_backtest
# Portfolio Simulator/Optimizer verwenden ebenfalls den Envelope Backtester intern (Annahme)
...
# Evaluator bleibt nützlich
from ltbbot.analysis.evaluator import evaluate_dataset
# Telegram zum Senden von Berichten
...
def run_single_analysis(start_date, end_date, start_capital):
    ...
    results_df["PnL %"] = results_df["PnL %"].map('{:.2f}%'.format)
    results_df["Max DD %"] = results_df["Max DD %"].map('{:.2f}%'.format)
    results_df["Win Rate %"] = results_df["Win Rate %"].map('{:.2f}%'.format)
    ...
    results_csv_path = os.path.join(results_dir, f'single_analysis_summary_{date.today()}.csv')
    try:
...
def run_portfolio_mode(is_auto: bool, start_date, end_date, start_capital):
    ...
    # --- Strategieauswahl (nur für manuellen Modus) ---
    selected_files = []
    ...
    if not is_auto:
        ...
        # Im Auto-Modus werden alle verfügbaren Konfigs an den Optimierer übergeben
        selected_files = available_filenames
    ...
    # portfolio_optimizer und simulator benötigen ein Dict: {strategy_id: {data: df, params: dict}}
    ...
    for filename in selected_files:
        ...
        if data is not None and not data.empty:
             strategies_data_for_sim[strategy_id] = {
                 'symbol': symbol, # Nützlich für Simulator
                 'timeframe': timeframe, # Nützlich für Simulator
                 'data': data,
    ...
    if is_auto:
        ...
        try:
            # Annahme: run_portfolio_optimizer akzeptiert das Format strategies_data_for_sim
            # und gibt ein Dict zurück, das 'optimal_portfolio' (Liste von IDs/Filenames)
            # und 'final_result' (Simulationsergebnis des besten Portfolios) enthält.
            logger.info("Starte automatische Portfolio-Optimierung...")
            results = run_portfolio_optimizer(start_capital, strategies_data_for_sim, start_date, end_date)
            ...
            if results and 'final_result' in results and 'optimal_portfolio' in results:
                ...
                for strat_id in optimal_portfolio_ids:
                     # Zeige Symbol/Timeframe statt nur Dateiname an
                ...
                # Equity Curve und Pfad für Export vorbereiten
                equity_df = final_report.get('equity_curve')
                report_csv_path = os.path.join(PROJECT_ROOT, 'optimal_portfolio_equity.csv') # Fester Name im Root
                ...
                # Optional: Speichere die Liste der optimalen Strategien in settings.json
                ...
                if save_optimal_to_settings:
                     try:
                         ...
                         # Erstelle neue Liste für active_strategies
        ...
        # --- Manuelle Simulation ---
        try:
            # Annahme: run_portfolio_simulation akzeptiert das Format strategies_data_for_sim
            # und gibt ein Dict mit den Simulationsergebnissen zurück.
            ...
            if results:
                ...
                mdd_date_str = mdd_date.strftime('%Y-%m-%d') if mdd_date else 'N/A'
                ...
                report_csv_path = os.path.join(PROJECT_ROOT, 'manual_portfolio_equity.csv') # Fester Name im Root
    ...
    if equity_df is not None and not equity_df.empty and report_csv_path:
        ...
        try:
            ...
            if export_cols:
                 equity_df[export_cols].to_csv(report_csv_path, index=False, float_format='%.8f') # Mehr Präzision für Equity
                 ...
                 try:
                     ...
                     if telegram_config.get('bot_token') and telegram_config.get('chat_id'):
                         ...
                         # Verwende die send_document Funktion aus dem utils Modul
                         send_document(
             ...
             logger.error(f"Fehler beim Exportieren der Equity Curve nach CSV: {e}")
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if start_capital_input <= 0:
         ...
         # Einfache Datumsvalidierung (optional)
