...
CONFIGS_DIR="src/ltbbot/strategy/configs"
...
CONFIG_COUNT=$(ls "$CONFIGS_DIR"/config_*_envelope.json 2>/dev/null | wc -l)
...
for f in "$CONFIGS_DIR"/config_*_envelope.json; do
    echo "  - $(basename "$f")"
done
echo ""

# Änderungen prüfen
git add "$CONFIGS_DIR"/config_*_envelope.json settings.json
...
git commit -m "Update: Envelope-Konfigurationen aktualisiert ($TIMESTAMP)"
